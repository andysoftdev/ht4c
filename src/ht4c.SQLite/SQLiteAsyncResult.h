...
namespace ht4c { namespace SQLite {
	...
	class SQLiteAsyncResult : public Common::AsyncResult {

		public:

			/// <summary>
			/// Creates a new SQLiteAsyncResult instance.
			/// </summary>
			...
			/// <returns>New SQLiteAsyncResult instance</returns>
			/// <remarks>To free the created instance, use the delete operator.</remarks>
			static SQLiteAsyncResult* create( Common::AsyncResultSink* asyncResultSink, size_t capacity = 0 );

			#ifndef __cplusplus_cli

			/// <summary>
			/// Initializes a new instance of the SQLiteAsyncResult class.
			/// </summary>
			/// <param name="capacity">Capacity in bytes of result queue. If zero then the queue capacity will be unbounded.</param>
			/// <remarks>Pure native constructor.</remarks>
			explicit SQLiteAsyncResult( size_t capacity );
			...
			/// <param name="capacity">Capacity in bytes of result queue. If zero then the queue capacity will be unbounded.</param>
			/// <remarks>Pure native constructor.</remarks>
			SQLiteAsyncResult( Common::AsyncResultSink* asyncResultSink, size_t capacity );
			...
			/// </summary>
			/// <param name="result">SQLite serialized result</param>
			/// <param name="asyncResult">Async result</param>
			/// <param name="asyncResultSink">Callback for asynchronous table scan operations</param>
			/// <param name="raiseException">If true the method raise an exception on error</param>
			/// <returns>true if succeeded</returns>
			/// <remarks>Pure native method.</remarks>
			//TODO static bool publishResult( Hypertable::SQLiteGen::ResultSerialized& result, Common::AsyncResult* asyncResult, Common::AsyncResultSink* asyncResultSink, bool raiseException );

			#endif

			/// <summary>
			/// Destroys the SQLiteAsyncResult instance.
			/// </summary>
			virtual ~SQLiteAsyncResult( );
		...
		private:
			...
			SQLiteAsyncResult( const SQLiteAsyncResult& ) { }
			SQLiteAsyncResult& operator = ( const SQLiteAsyncResult& ) { return *this; }
